...
export default function OrdersPage() {
  ...
  const stats = React.useMemo(() => {
    ...
    const cancelled = orders.filter((o: any) => String(o?.status).toLowerCase() === "cancelled").length;
    return {
      total: orders.length,
      delivered,
      processing,
    ...
    <section className="flex flex-col gap-6">
      <header className="flex flex-col gap-4 rounded-3xl border border-white/70 bg-white/95 p-6 shadow-[0_30px_100px_-70px_rgba(5,150,145,0.45)] sm:p-8">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#02C1BE]">Purchases</p>
            <h1 className="text-3xl font-bold text-slate-900">Your Orders</h1>
            <p className="text-sm text-slate-500">Track deliveries, review invoices, and manage returns in one place.</p>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: "Total Orders", value: stats.total, tone: "text-[#02C1BE]" },
          ].map((stat) => (
            <div key={stat.label} className="rounded-2xl border border-white/80 bg-white/90 p-4 shadow-[0_18px_40px_-30px_rgba(5,150,145,0.3)]">
      ...
      {isLoading ? (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="rounded-3xl border border-white/70 bg-white/90 p-5 shadow-[0_30px_90px_-70px_rgba(5,150,145,0.45)]">
        ...
        <div className="flex flex-col items-center justify-center gap-4 rounded-3xl border border-dashed border-[#02C1BE]/30 bg-[#f7fffe] py-16 text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-900">You haven’t placed any orders yet</h3>
          <p className="max-w-sm text-sm text-slate-500">
            Start exploring new arrivals and add products to your cart to see them appear here.
          </p>
          <Link
            href="/"
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-900">{slug || orderId}</p>
                    </div>
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${statusTone}`}>{status || "Processing"}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    {previewItems.length === 0 ? (
                      ...
                      <div className="flex -space-x-2">
                        {previewItems.map((it: any, idx: number) => {
                          ...
                          return (
                            <div key={idx} className="flex h-12 w-12 items-center justify-center rounded-xl border border-white bg-white shadow">
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                              <img src={img || "/next.svg"} alt={it?.name || "Item"} className="h-10 w-10 rounded-lg object-cover" />
                            </div>
                          );
                        })}
                      </div>
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-slate-800">{previewItems[0]?.name || "Order items"}</p>
                      <p className="text-xs text-slate-500">{quantityTotal} items · {totalDisplay}</p>
                    </div>
                  </div>
                </div>
          ...
          {selectedOrder ? (
            <div className="space-y-4 sm:space-y-6 py-4 pb-8">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Badge className={getStatusColor(String(selectedOrder.status || ""))}>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <div className="rounded-2xl border border-white/60 bg-white p-4 shadow-sm">
                  ...
                  <p className="mt-2 text-sm font-semibold text-slate-900">{selectedOrder?.customer?.fullName}</p>
                  <p className="text-xs text-slate-500">{selectedOrder?.customer?.phone}</p>
                  <p className="truncate text-xs text-slate-500">{selectedOrder?.customer?.email}</p>
                </div>
                <div className="rounded-2xl border border-white/60 bg-white p-4 shadow-sm">
                  <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Delivery</p>
                  <p className="mt-2 text-sm font-semibold text-slate-900 capitalize">{selectedOrder?.delivery?.method?.toLowerCase?.()}</p>
                  <p className="text-xs text-slate-500">{selectedOrder?.customer?.address}</p>
                  <p className="text-xs text-slate-500">{selectedOrder?.customer?.upazila}, {selectedOrder?.customer?.district}, {selectedOrder?.customer?.division}</p>
                </div>
                <div className="rounded-2xl border border-white/60 bg-white p-4 shadow-sm">
                  <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Payment</p>
                  <p className="mt-2 text-sm font-semibold text-slate-900">{selectedOrder?.payment?.method}</p>
                  {selectedOrder?.couponCode && (
                    <p className="mt-1 text-xs text-emerald-600">Coupon: {selectedOrder.couponCode}</p>
                  )}
                </div>
              </div>
              ...
              {/* Totals */}
                ...
                <div className="flex justify-between">
                    <span>Subtotal</span>
                    <span className="font-medium text-slate-900">৳ {Number(selectedOrder?.totals?.subtotal ?? 0).toFixed(2)}</span>
                </div>
                ...
                <div className="flex justify-between">
                  <span>Shipping</span>
                  <span className="font-medium text-slate-900">৳ {Number(selectedOrder?.totals?.shippingTotal ?? selectedOrder?.delivery?.charge ?? 0).toFixed(2)}</span>
                </div>
