    ...
    <div className="min-h-screen bg-gray-50 py-6 sm:py-8">
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-14">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
                  <Link href={`/category/${resolvedParams.category}`} className="hover:text-[#02C1BE]">
                    {context?.category?.name || resolvedParams.category}
                  </Link>
                  <span>/</span>
                  <span className="text-gray-900">{context?.subcategory?.name || resolvedParams.subcategory}</span>
                </div>
                ...
                <p className="mt-2 text-sm text-slate-600 max-w-2xl">
                  Discover premium {context?.subcategory?.name || resolvedParams.subcategory} products
                </p>
              </div>
              {/* Sort Selector */}
        ...
        {filteredItems.length > 0 ? (
          ...
          <div className="rounded-3xl border border-gray-100 bg-white shadow-sm px-4 sm:px-8 py-12 text-center">
            <p className="text-gray-500 text-lg">No active products found in this subcategory.</p>
          </div>
        )}
      </div>
    </div>
